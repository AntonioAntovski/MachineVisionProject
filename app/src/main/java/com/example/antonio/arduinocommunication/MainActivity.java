            ...
            ImageReader.OnImageAvailableListener readerListener = new ImageReader.OnImageAvailableListener() {
                ...
                public void onImageAvailable(ImageReader reader) {
                    ...
                    try {
                        ...
                        buffer.get(bytes);
//                        save(bytes);
                        process_image(BitmapFactory.decodeByteArray(bytes, 0, bytes.length, null));
                    } finally {
                        if (image != null) {
                            image.close();
                        }
...
//                }
    ...
    public void process_image(Bitmap imageBitmap){
        ...
        Log.d("process_image", "c = " + maxC);

        try {
            sendViaBluetooth(maxC);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }

    public void sendViaBluetooth(String c) throws IOException {
        BluetoothAdapter mBluetoothAdapter = BluetoothAdapter.getDefaultAdapter();
        if(mBluetoothAdapter != null){
            if(mBluetoothAdapter.isEnabled()){
                Set<BluetoothDevice> pairedDevices = mBluetoothAdapter.getBondedDevices();
                Object[] devices = pairedDevices.toArray();
                BluetoothDevice device = (BluetoothDevice) devices[9];
